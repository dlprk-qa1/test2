...
resource "azurerm_container_registry" "cr-finance" {
  provider = azurerm.mission-critical-dev 
...
resource "azurerm_network_interface" "nic-finance-vm2" {
  provider = azurerm.mission-critical-dev 
  name                = "nic-finance-${var.environment}-${var.region}-2"
  location            = azurerm_resource_group.rg-finance.location
  resource_group_name = azurerm_resource_group.rg-finance.name
  ...
  ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.snet-finance.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id = azurerm_public_ip.pip-finance2.id 
  }
}
...
resource "azurerm_network_interface_security_group_association" "nsg-association-finance-vm2" {
  provider = azurerm.mission-critical-dev 
  network_interface_id      = azurerm_network_interface.nic-finance-vm2.id
  network_security_group_id = azurerm_network_security_group.nsg-finance-vm1.id
}
...
resource "azurerm_linux_virtual_machine" "vm-finance-2" {
  provider = azurerm.mission-critical-dev 
  name                  = "vm-finance-${var.environment}-${var.region}-2"
  location              = azurerm_resource_group.rg-finance.location
  resource_group_name   = azurerm_resource_group.rg-finance.name
  network_interface_ids = [azurerm_network_interface.nic-finance-vm2.id]
  size               = "Standard_A1_v2"
...
source_image_reference {
  ...
  os_disk {
    name                 = "osdisk-finance2"
    caching              = "ReadWrite"
    storage_account_type = "Standard_LRS"
  }


  computer_name                   = "finance-vm2"
  admin_username                  = "azureuser"
  disable_password_authentication = true
...
resource "azurerm_public_ip" "pip-finance2" {
  provider = azurerm.mission-critical-dev 
  name                = "pip-finance-${var.environment}-${var.region}-2"
  resource_group_name = azurerm_resource_group.rg-finance.name
  location            = azurerm_resource_group.rg-finance.location
  allocation_method   = "Dynamic"
...
resource "azurerm_role_assignment" "roleassingment-ecomm-vm" {
  provider = azurerm.mission-critical-dev 
  scope                = "/subscriptions/${var.subscription}"
  role_definition_name = "Virtual Machine Contributor"
  principal_id         = azurerm_linux_virtual_machine.vm-ecomm-1.identity.0.principal_id
...
##### Automating service account access keys creation to trigger ZPC Suspicious Activity threat:

data "azuread_application" "current-ad-app" {
  display_name = "max-terraformer-reader"
}


resource "azuread_application_password" "current-password" {
  application_object_id = data.azuread_application.current-ad-app.object_id
  display_name = "attack beachhead"
}

resource "azuread_application_password" "current-password1" {
  application_object_id = data.azuread_application.current-ad-app.object_id
  display_name = "attack beachhead-2"
}

resource "azuread_application_password" "current-password2" {
  application_object_id = data.azuread_application.current-ad-app.object_id
  display_name = "attack beachhead-3"
} 