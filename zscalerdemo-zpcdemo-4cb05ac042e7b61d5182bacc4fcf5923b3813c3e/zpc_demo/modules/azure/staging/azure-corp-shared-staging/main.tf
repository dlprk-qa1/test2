...
data "azurerm_subscription" "corp-shared-staging" {
}
...
resource "azurerm_container_registry" "cr-hr" {
  provider = azurerm.corp-shared-staging 
...
resource "azurerm_role_assignment" "roleassingment-hr" {
  provider = azurerm.corp-shared-staging 
  scope                = data.azurerm_subscription.corp-shared-staging.id
...
resource "azurerm_network_interface" "nic-hrweb-vm1" {
  provider = azurerm.corp-shared-staging 
  name                = "nic-hrweb-${var.environment}-${var.region}"
  location            = azurerm_resource_group.rg-hr.location
  resource_group_name = azurerm_resource_group.rg-hr.name
  ...
  ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.snet-hr.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id = azurerm_public_ip.pip-hrweb.id 
  }
}
...
resource "azurerm_network_security_group" "nsg-hrweb-vm1" {
  provider = azurerm.corp-shared-staging 
  name                = "nsg-hrweb-${var.environment}-${var.region}"
  location            = azurerm_resource_group.rg-hr.location
  resource_group_name = azurerm_resource_group.rg-hr.name
...
resource "azurerm_network_interface_security_group_association" "nsg-association-hrweb-vm1" {
  provider = azurerm.corp-shared-staging 
  network_interface_id      = azurerm_network_interface.nic-hrweb-vm1.id
  network_security_group_id = azurerm_network_security_group.nsg-hrweb-vm1.id
}
...
resource "azurerm_linux_virtual_machine" "vm-hrweb-1" {
  provider = azurerm.corp-shared-staging 
  name                  = "vm-hrweb-${var.environment}-${var.region}-1"
  location              = azurerm_resource_group.rg-hr.location
  resource_group_name   = azurerm_resource_group.rg-hr.name
  network_interface_ids = [azurerm_network_interface.nic-hrweb-vm1.id]
  size               = "Standard_A1_v2"
...
resource "azurerm_public_ip" "pip-hrweb" {
  provider = azurerm.corp-shared-staging 
  name                = "pip-hrweb-${var.environment}-${var.region}"
  resource_group_name = azurerm_resource_group.rg-hr.name
  location            = azurerm_resource_group.rg-hr.location
