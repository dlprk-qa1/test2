...
resource "google_service_account_key" "new-key" {
  service_account_id = google_service_account.bigqowner.name
}

resource "google_service_account_key" "new-key-aug-2022" {
  service_account_id = google_service_account.bigqowner.name
}
...
resource "google_kms_key_ring" "erp_key_ring" {
  project  = var.project
  name     = "erp_key_ring"
  location = var.region
}

resource "google_kms_crypto_key" "erp_crypto_key" {
  name     = "erp-crypto-key"
  key_ring = google_kms_key_ring.erp_key_ring.id
}
